...
class ParallelTextReader:    
    ...
    def next(self):
        ...
        if self.source_max_length == None and self.target_max_length == None:
            src_lines = list(itertools.islice(self.target_dataset, self.batchsize))   
            trg_lines = list(itertools.islice(self.source_dataset, self.batchsize)) 
            #print [line.split() for line in lines]
            if lines == []:
                self.reset()
                raise StopIteration
            samples = [([src_token for token in src_line.decode(self.encoding).strip().split()], 
                        [trg_token for token in trg_line.decode(self.encoding).strip().split()])
